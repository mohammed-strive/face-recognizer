...
def read_data(image_paths, label_list, image_size, batch_size,
    max_nrof_epochs, num_threads, shuffle, random_flip,
    ...
    for _ in range(num_threads):
        ...
        image_batch, label_batch = tf.train.batch_join(image_labels, batch_size=batch_size,
...
def get_image_paths_and_labels(dataset):
    ...
    for i in range(int(len(dataset))):
        image_paths_flat.append(dataset[i].image_paths)
        labels_flat.extend([i]*len(dataset[i].image_paths))
...
def split_dataset(dataset, split_ratio=0.8):
    ...
    for cls in dataset:
        ...
        train_set.append(ImageClass(cls.name, paths[split:-1]))
